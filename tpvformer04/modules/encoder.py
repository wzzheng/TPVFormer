...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, bev_h, bev_w, bev_z, pc_range=None, 
        ...
        ref_3d_zh = self.get_reference_points(bev_z, bev_h, pc_range[3]-pc_range[0], num_points_in_pillar[1], '3d', device='cpu')
        ref_3d_zh = ref_3d_zh.permute(3, 0, 1, 2)[[2, 0, 1]]
        ref_3d_zh = ref_3d_zh.permute(1, 2, 3, 0)

        ref_3d_wz = self.get_reference_points(bev_w, bev_z, pc_range[4]-pc_range[1], num_points_in_pillar[2], '3d', device='cpu')
    ...
    def forward(self,
        ...
        shift_ref_2d = ref_2d_hw.clone() 

        # (num_query, bs, embed_dims) -> (bs, num_query, embed_dims)
        bev_query = [bev.permute(1, 0, 2) for bev in bev_query]
        bev_pos = [pos.permute(1, 0, 2) for pos in bev_pos if pos is not None]
            ...
            if layer == 'self_attn':
                ...
                query_0 = self.attentions[attn_index](
                    ...
                    spatial_shapes=torch.tensor(
                        [[bev_h, bev_w]], device=query[0].device),
